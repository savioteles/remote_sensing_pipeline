...
inputFile = "/media/savio/67E3BA5B4FBF63B1/modis_ndvi/400.txt"
...
bfit = sapply(1:nrow(rows), function(i){
  ...
  if(all(is.na(values))) {c(NA,NA,NA)} else{
    pixel = as.numeric(values)
    pixel[is.na(pixel)] = 0
    dat = bfastts(pixel[2:length(pixel)], dates, type = c("16-day"))
    ...
    return(c(as.character(rows[i,][1,1]), as.numeric(lastBreakJulianDate), as.numeric(slope)))
  }
}
)
