...
#################################################################################
#
# Claudinei Oliveira dos Santos Biologo | Mestre em Ecologia | Doutorando em
# Ciencias Ambientais IPAM- Instituto de Pesquisa Ambiental da Amazonia |
# Assistente de Pesquisa LAPIG- Laboratorio de Processamento de Imagens e
# Geoprocessamento | Doutorando claudineisantosnx@gmail.com |
# claudinei.santos@ipam.org.br
#
#################################################################################
#################################################################################
...
###
###
load("/home/savio/R_workspace/bfast/ndvi_sample_nobreak.rdata")
head(mm2)
#load("S:\\RASTER\\qualidade_pastagem\\DATASAN\\NDVI\\all_data_parts_files\\NDVI_374_layers_parte_33.Rdata")

m = as.data.frame(mm2[, -1])
m = as.matrix(m)
rm(mm2)
...
Rprof("bfast.out")
sart.timet = Sys.time()
bfit = apply(mm1, 1, function(x){
  
  if(all(is.na(x))) {c(NA,NA)} else{
    
    pixel = as.numeric(x)
    pixel[is.na(pixel)] = 0
    
    bf = bfast(bfastts(pixel, dates, type = c("16-day")), h=0.06099599, season="harmonic", max.iter=1)
    
    #x11();plot(bf)
    
    breaks = bf$output[[1]]$Vt.bp
    trendCmp = bf$output[[1]]$Tt
    
    lastBreak = if(!bf$nobp$Vt) breaks[length(breaks)] else 1
    ...
    return(c(as.numeric(lastBreakJulianDate), as.numeric(slope)))
