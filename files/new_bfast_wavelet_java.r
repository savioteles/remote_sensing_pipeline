...
inputFile = "/home/savio/R_workspace/ndvi_tanguro/DADOS/bfast_input/bfast_input_all.txt"
...
bfit = sapply(1:nrow(rows), function(i){
  ...
  if(all(is.na(values))) {c(NA,NA,NA,NA)} else{
    ...
    #half of original data size
    dwtobj <- dwt(pixel[2:length(pixel)], filter = "d2", n.levels=2)
    wavelet_data = dwtobj@V$V1
    #run bfast using wavelet
    bf = bfast(bfastts(wavelet_data[,1], dates[1:192], type = c("16-day")), h=h, season="harmonic", max.iter=1)
    ...
    if(all(is.na(breaks)) || bf$nobp$Vt) {
      lastBreak = 1
    ...
    output_break_dates = paste(breaks_dates, collapse=";")
    ...
    return(c(as.character(rows[i,][1,1]), time, as.character(output_break_dates), as.numeric(slope)))
  }
}
)
