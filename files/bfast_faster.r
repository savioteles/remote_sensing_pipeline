...
bfast_luis=function(dat){                      ## dat deve ser uma serie temporal.
  # plot(dat)
  time.dat=as.vector(time(dat))               ## variável tempo.
  stl_dat=stl(dat, "per",robust=TRUE)
  dat=stl_dat$time.series[,2]+stl_dat$time.series[,3]
  ti <- time(dat)
  bp.dat <- breakpoints(dat ~ 1,h=24/length(dat))       ## estima os break points.OBS: usa um modelo mais simples que o bfast usual. 
  fm1 <- lm(dat ~ time.dat*breakfactor(bp.dat, breaks = length(bp.dat$breakpoints))) ## estima a tendência linear
  plot(dat)
  lines(ts(fitted(fm1),start=start(dat),frequency = frequency(dat)), col = 4) ### plot da tendência.
  lines(bp.dat)                               ## plot breaks points.
  return (bp.dat)
}
...
load("/home/savio/R_workspace/bfast/ndvi_sample.RData")
m = as.data.frame(ndvi[, -1])
m = as.matrix(m)
rm(ndvi)

pixel = as.numeric(m[1,])
pixel[is.na(pixel)] = 0

ts = bfastts(pixel[1:374], dates[1:374], type = c("16-day"))

sart.timet = Sys.time()
bf_luis = bfast_luis(ts)
time = Sys.time() - sart.timet  
cat(time)
plot(ts)

sart.timet = Sys.time()
bf = bfast(ts, h=0.06099599, season="harmonic", max.iter=1)
time = Sys.time() - sart.timet  
cat(time)

breaks_luis = unlist(bf_luis['breakpoints'])
lines(bf_luis, col="red")
breaks_bfast = bf$output[[1]]$bp.Vt
lines(breaks_bfast, col="blue")

wt <- dwt(pixel)
t = as.numeric(unlist(wt@W['W2']))
ts2 = bfastts(ts, dates, type = c("16-day"))
bf = bfast(ts1, h=0.06099599, season="harmonic", max.iter=1)

breaks_bfast = bf$output[[1]]$bp.Vt
lines(breaks_bfast, col="blue")

dwtobj <- dwt(pixel, filter = "d2", n.levels=2)
teste1 = dwtobj@V$V1
...
ts1 = bfastts(teste1[,1], dates[1:187], type = c("16-day"))
sart.timet = Sys.time()
bf = bfast(ts1, h=0.05208556, season="harmonic", max.iter=1)
time = Sys.time() - sart.timet  
cat(time)
plot(bf)

ts2 = bfastts(pixel, dates, type = c("16-day"))
sart.timet = Sys.time()
bf1 = bfast(ts2, h=0.06099599, season="harmonic", max.iter=1)
time = Sys.time() - sart.timet  
cat(time)
plot(bf1)


tsl1 = bfastts(teste1[,1], dates[1:187], type = c("16-day"))
sart.timet = Sys.time()
bf_luis = bfast_luis(tsl1)
time = Sys.time() - sart.timet  
cat(time)


tsl2 = bfastts(pixel, dates, type = c("16-day"))
sart.timet = Sys.time()
bf_luis1 = bfast_luis(tsl2)
time = Sys.time() - sart.timet  
cat(time)
